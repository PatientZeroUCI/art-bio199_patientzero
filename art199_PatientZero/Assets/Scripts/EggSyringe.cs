...
public class EggSyringe : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject PokeZone;  // Where the syringe has to enter the egg
    [SerializeField]
    private GameObject EggCenter;  // Where the syringe has to poke
    ...
    private void OnTriggerEnter(Collider obj)
    {
        ...
        else if ((obj.gameObject == Egg) && !goodPoke)
        {
            Debug.Log("Bad");
            badPoke = true;
        ...
        // When needle enter the PokeZone first then hits the center
        else if((obj.gameObject == EggCenter) && goodPoke)
        {
            Debug.Log("Good");
